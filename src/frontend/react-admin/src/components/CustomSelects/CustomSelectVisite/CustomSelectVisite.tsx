...
import { VisiteResponse } from '../../../generated';


const CustomSelectVisite: React.FC<CustomSelectInputProps> = ({
  inputLabel,
  inputPlaceholder,
  required,
  wrapperStyle,
  ...
  const handleOptionSelect = (selectedValue: VisiteResponse) => {
    setInputValue(selectedValue.motif + ' -- ' + selectedValue.user?.nom + ' ' + selectedValue.user?.prenom );
        ...
        <ul>
          {isDropdownOpen && inputValue !== '' && (
            ...
            >
              ...
              >
                {matchList.length > 0 ? matchList.map((option: VisiteResponse, index) => (
                  <li
                  ...
                  >
                    {option.motif + ' -- ' + option.user?.prenom + ' ' + option.user?.nom + ' -- ' + option.visiteur?.prenom + ' ' + option.visiteur?.nom}
                  </li>
                )) :
                  <li
...
export default CustomSelectVisite;
